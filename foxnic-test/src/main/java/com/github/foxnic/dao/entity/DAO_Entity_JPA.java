...
import com.github.foxnic.dao.base.jpa.RelationJPA;
...
import com.github.foxnic.sql.entity.EntityUtil;
  ...
  * 针对各种值类型测测试
  * */
public class DAO_Entity_JPA  extends TableDataTest{
 
	@Test
	public void test_getTableName() {
		
		//从注解读取
		String table=EntityUtil.getAnnotationTable(NewsJPA.class);
		//从缓存读取
		table=EntityUtil.getAnnotationTable(NewsJPA.class);
		//
	...
	public void test_insert_update() {
		NewsJPA news=new NewsJPA();
		 ...
		 //转换为实体
		 NewsJPA news2=rs.getRcd(0).toEntity(NewsJPA.class);
	...
	public void test_save() {
		NewsJPA news=new NewsJPA();
		 ...
		 RcdSet rs=dao.query("select * from "+normalTable+" where code=?","AA");
		 assertTrue(rs.size()==1);
		 NewsJPA newsdb=rs.getRcd(0).toEntity(NewsJPA.class);
	...
	public void test_pk_table() {
		 ...
		 RelationJPA rel=new RelationJPA();
		 rel.setBillId(9);
		 rel.setOwnerId(1);
		 rel.setType("shop");
