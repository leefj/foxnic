	...
	public void buildControllerMethod(FileBuilder builder, Context ctx, CodeBuilder code) {
		...
		if(ctx.isEnableSwagger()) {
			...
			for (DBColumnMeta pk : pks) {
				...
				String apiImplicitParamName=ctx.getDefaultVOMeta().getSimpleName()+"."+pk.getColumn().toUpperCase();
			...
			for (DBColumnMeta pk : notNulls) {
				code.ln(1,"@NotNull(name = "+ctx.getDefaultVOMeta().getSimpleName()+"."+pk.getColumn().toUpperCase()+")");
